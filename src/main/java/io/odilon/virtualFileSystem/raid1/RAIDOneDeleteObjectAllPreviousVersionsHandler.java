...
public class RAIDOneDeleteObjectAllPreviousVersionsHandler extends RAIDOneTransactionObjectHandler {
	...
	protected void delete() {
		...
		if (commitOK) {
			try {
				/**
				 * after the TRX commit. It is used to clean temp files, if the system crashes
				 * those temp files will be removed on system startup
				 */
				getSchedulerService().enqueue(getVirtualFileSystemService().getApplicationContext().getBean(AfterDeleteObjectServiceRequest.class, operation.getOperationCode(), meta, meta.getVersion()));
			} catch (Exception e) {
