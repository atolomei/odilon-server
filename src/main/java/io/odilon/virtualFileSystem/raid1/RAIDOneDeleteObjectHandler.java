...
public class RAIDOneDeleteObjectHandler extends RAIDOneTransactionObjectHandler {
	...
	private void onAfterCommit(VirtualFileSystemOperation op, ObjectMetadata meta, int headVersion) {
		try {
			getVirtualFileSystemService().getSchedulerService().enqueue(getVirtualFileSystemService().getApplicationContext().getBean(AfterDeleteObjectServiceRequest.class, op.getOperationCode(), meta, headVersion));
		} catch (Exception e) {
			logger.error(e, SharedConstant.NOT_THROWN);
		}
	}
