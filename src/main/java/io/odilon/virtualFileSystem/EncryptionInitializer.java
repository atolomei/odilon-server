 ...
 * </p>
 * <p>
 * An instance of this class is created by the <b>initialize encryption</b>
 * script:<br/>
 * ./bin/enable-encryption.sh (Linux) or .\bin\enable-encryption.bat (Windows)
 * <br/>
 * <br/>
 * and also by the <b>rekey</b> script: <br/>
 * ./rekey-encryption.sh -m masterkey (Linux) or rekey-encryption.bat -m
 * masterkey (Windows).
 * </p>
 * 
 * <p>
 * Both scripts start up the application with one (initialize) or two (rekey)
 * parameters: <br/>
 * <br/>
 * To initialze encryption:<br/>
 * -DinitializeEncryption=true<br/>
 * <br/>
 * The rekey script also adds this parameter (the master key must be provided to
 * generate a new encryption key):<br/>
 * -DmasterKey=ab5d6e5d6f9523eba7208b5b3ec875ba<br/>
 * </p>
 * 
 * <p>
 * See <a href=
 * "https://odilon.io/configuration-advanced.html">https://odilon.io/configuration-advanced.html</a>
 * </p>
 * 
...
public class EncryptionInitializer extends BaseObject {
    ...
    public void notInitializedError() {
        startuplogger.info("");
        startuplogger
                .info("The server is configured to use encryption (ie. 'encryption.enabled=true' in file 'odilon.properties')");
        startuplogger.info("but the encryption service has not been initialized yet.");
        startuplogger.info("");
        startuplogger.info("You have to either:");
        startuplogger
                .info("a. Disable encryption in 'odilon.properties' by changing the variable to 'encryption.enabled=false' or");
        startuplogger.info("");
    ...
    private synchronized void initializeEnc() {
        ...
        byte[] encKey = new byte[EncryptionService.AES_KEY_SIZE_BITS / VirtualFileSystemService.BITS_PER_BYTE]; // 16 bytes -> 2
                                                                                                                // ASCII chars per
                                                                                                                // byte -> 32 ASCII
                                                                                                                // chars
        byte[] iv = new byte[EncryptionService.AES_IV_SIZE_BITS / VirtualFileSystemService.BITS_PER_BYTE]; // 12 bytes -> 2 ASCII
                                                                                                           // chars per byte -> 24
                                                                                                           // ASCII chars
        ...
        /**
         * the encryption key encoded in ASCII is = AES KEY 32 ASCII characters + IV 24
         * ASCIII characteres = 56 ASCII chars
         */

        startuplogger.info("encryption.key = " + ByteToString.byteToHexString(encKeyIV));
        startuplogger.info(
                "The encrytion key must be added to the 'odilon.properties' file in variable 'encryption.key' as printed above.");
        startuplogger.info("");
        ...
        startuplogger.info("");
        startuplogger.info(
                "The master key is used internally and it is secret, it is NOT required in 'odilon.properties' or anywhere else.");
    ...
    private void rekeyNotIntializedError() {
        ...
        startuplogger.info("You have to initialize the Encryption Service by executing '" + getEnableEncryptionScriptName()
                + "', Odilon will generate the encryption keys");
        startuplogger.info("");
        startuplogger.info("The server will shutdown now.");
