...
public class ObjectControllerStatic extends BaseApiController {
	...
	public ObjectControllerStatic(ObjectStorageService objectStorageService, VirtualFileSystemService virtualFileSystemService, SystemMonitorService monitoringService, TrafficControlService trafficControlService, TokenService tokenService,
	...
	@ResponseBody
	public ResponseEntity<InputStreamResource> getStaticObjectStream(@RequestParam("token") String stringToken, @RequestHeader(value = "Range", required = false) String rangeHeader) {
		...
		try {
			...
			// logger.debug(object.getObjectMetadata().getFileName()+ " "+
