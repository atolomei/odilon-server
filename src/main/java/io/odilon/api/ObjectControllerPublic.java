...
public class ObjectControllerPublic extends BaseApiController {
	...
	public ObjectControllerPublic(ObjectStorageService objectStorageService, VirtualFileSystemService virtualFileSystemService, SystemMonitorService monitoringService, TrafficControlService trafficControlService, TokenService tokenService,
	...
	@ResponseBody
	public ResponseEntity<InputStreamResource> getObjectStream(@PathVariable("bucketName") String bucketName, @PathVariable("objectName") String objectName, @PathVariable("fileName") String fileName,
		...
		try {
			...
			if (rangeHeader != null && rangeHeader.startsWith("bytes=")) {
				...
				return ResponseEntity.status(HttpStatus.PARTIAL_CONTENT).headers(responseHeaders).contentType(contentType).contentLength(contentLength)
						.cacheControl(CacheControl.maxAge(ServerConstant.ONE_YEAR_SECS, TimeUnit.SECONDS).cachePublic().immutable()).body(new InputStreamResource(new LimitedInputStream(in, contentLength)));
			} else {
				// Full file
				...
				if (fileLength == -1) {
					return ResponseEntity.ok().headers(responseHeaders).contentType(contentType).cacheControl(CacheControl.maxAge(ServerConstant.ONE_YEAR_SECS, TimeUnit.SECONDS).cachePublic().immutable()).body(new InputStreamResource(in));
				} else {
					return ResponseEntity.ok().headers(responseHeaders).contentType(contentType).contentLength(fileLength).cacheControl(CacheControl.maxAge(ServerConstant.ONE_YEAR_SECS, TimeUnit.SECONDS).cachePublic().immutable())
